...
public class OneWayWall : MonoBehaviour
{
    ...
    {
        if (_player._currentHorizontalSpeed < 0)
            _unlockBuffer = Time.time + UnlockTime; // time boxCollider will be disabled
        
        _collider.enabled = _player._currentHorizontalSpeed <= 0 && Time.time >= _unlockBuffer; // disable collider until condition is met
